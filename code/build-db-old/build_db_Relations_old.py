...
logging.basicConfig(filename='build_db_relations.log', encoding='utf-8', level=logging.INFO)
...
def update_relations_table(sqlConnection, SystemName, FromPackageId, ToPackageId):
    try:
        sqlite_check_row_query = '''
            SELECT RelationId
            From Relations
            WHERE SystemName=%s AND FromPackageId=%s AND ToPackageId=%s;
        '''

        cursor = sqlConnection.cursor()
        logging.debug(f"update_relations_table()")
        cursor.execute(sqlite_check_row_query, (SystemName, FromPackageId, ToPackageId))
        check_result = cursor.fetchone()
        if check_result is None:
            # The record could not be found in the database
            logging.debug(f"The {SystemName}: {FromPackageId} -> {ToPackageId} is not already in the database")
            update_query = '''
                INSERT INTO Relations(SystemName, FromPackageId, ToPackageId)
                Values(%s, %s, %s);
            '''
            cursor.execute(update_query, (SystemName, FromPackageId, ToPackageId))
        else:
            # The record is already in the database
            logging.error(f"This <parent, child> pair should not already be in the database")
            None
                ...
                with open(file_path, 'r', encoding="utf-8") as f:
                    ...
                    try:
                        for i, line in enumerate(csvreader):
                            ...
                            update_relations_table(sqlConnection, system_name, parent_package_id, child_package_id)
                    except csv.Error:
