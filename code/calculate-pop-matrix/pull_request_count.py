...
def get_pull_count(client, repo_name):
    repo = client.get_repo(repo_name)
    pulls = repo.get_pulls(direction='desc', state='all').get_page(0)
    # print (pulls)
    for pull in pulls:
        return pull.number
    return 0

def get_pull_count_with_time(client, repo_name):
    
    # Starting time
    st = time.time()
    pull_count = get_pull_count(client=client, repo_name=repo_name)
    # Ending time
    et = time.time()
    return pull_count, (et - st)
...
def open_csv_file():
    ...
    csv_file = open("pull_count.csv", "w")
    header = ["Repository", "Pull Count", "Repo URL"]
...
def func(csv_writer, client, repo_name):
    ...
    """
    pull_count, time_elapsed = get_pull_count_with_time(client=client, repo_name=repo_name)
    csv_writer.writerow({
