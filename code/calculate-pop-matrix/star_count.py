...
def get_star_count(client, repo_name):
    repo = client.get_repo(repo_name)
    return repo.stargazers_count

def get_star_count_with_time(client, repo_name):
    
    # Starting time
    st = time.time()
    star_count = get_star_count(client=client, repo_name=repo_name)
    # Ending time
    et = time.time()
    return star_count, (et - st)
...
def open_csv_file():
    ...
    csv_file = open("star_count.csv", "w")
    header = ["Repository", "Star Count", "Repo URL"]
...
def func(csv_writer, client, repo_name):
    ...
    """
    star_count, time_elapsed = get_star_count_with_time(client=client, repo_name=repo_name)
    csv_writer.writerow({
