...
def get_fork_count(client, repo_name):
    repo = client.get_repo(repo_name)
    return repo.forks_count

def get_fork_count_with_time(client, repo_name):
    
    # Starting time
    st = time.time()
    fork_count = get_fork_count(client=client, repo_name=repo_name)
    # Ending time
    et = time.time()
    return fork_count, (et - st)
...
def open_csv_file():
    ...
    csv_file = open("fork_count.csv", "w")
    header = ["Repository", "Fork Count", "Repo URL"]
...
def func(csv_writer, client, repo_name):
    ...
    """
    fork_count, time_elapsed = get_fork_count_with_time(client=client, repo_name=repo_name)
    csv_writer.writerow({
